...
for s=1:nsubj
        ...
        for h=1:nhem
                ...
                warp_sphere=[warpDir,'standard',num2str(mmRes),'mm_',sub,'_',hem,'.connnorm.asc'];
                ...
                make_warped_nonsphere(origCoords,warpTris,nearest_nodes,[warpPrefix,'.smoothwm.connnorm.asc']);
                origCoords = read_fs_ascii_mesh(orig_pial);
                make_warped_nonsphere(origCoords,warpTris,nearest_nodes,[warpPrefix,'.pial.connnorm.asc']);
                
                % make a spec file with warp_sphere, warp_pial, warp_smoothm in it
                fid = fopen([warpPrefix,'.spec'],'w');
                fprintf(fid,'# mesh generated by make_warped_meshes.m, part of connnorm\n');
...
T = size(warpTris,2);
fprintf(fid,'# made by make_warped_meshes.m, part of connnorm\n');
fprintf(fid,'%d %d\n',N,T);
for n = 1:N
    fprintf(fid,'%f %f %f 0\n',...
