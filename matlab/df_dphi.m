function dVals = df_dphi(spherCoord1, spherCoord2, resolution, gds)
% FUNCTION dVals = df_dphi(spherCoord1, spherCoord2, resolution [, gds])
...
if size(spherCoord1, 2) == 1
	...
	if size(spherCoord1, 2) ~= size(spherCoord2, 2)
		error('df_dphi:  size of sphercoord1 and sphercoord2 must match in multi submode');
	end
end	

h = 0.0201*resolution;

% It's ok if spherCoord1 is 2 x 1
...
if strcmp(subMode, 'single')
	dVals(nonZeroLocs) = -20*(1 - 2*sin(gds(nonZeroLocs)/2)/h).^3.*(sin(phis1).*cos(phis2(nonZeroLocs)) - cos(phis1).*sin(phis2(nonZeroLocs)).*cos(thetas1 - thetas2(nonZeroLocs)))/h^2;
else
	dVals(nonZeroLocs) = -20*(1 - 2*sin(gds(nonZeroLocs)/2)/h).^3.*(sin(phis1(nonZeroLocs)).*cos(phis2(nonZeroLocs)) - cos(phis1(nonZeroLocs)).*sin(phis2(nonZeroLocs)).*cos(thetas1(nonZeroLocs) - thetas2(nonZeroLocs)))/h^2;
end