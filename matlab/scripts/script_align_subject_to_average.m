function warps = script_align_subject_to_average(partNum, lambda, pctEnergy)
...
if outDir(end) ~= '/'
	...
	alignDir = [outDir, 'alignmentSingleSubjToAvg_', experiment, '_', num2str(lambda_areal), '_', num2str(pctEnergy), '_partNum', num2str(partNum), '/'];
    if isdir(alignDir) == 0
        mkdir(alignDir);
    end
...
%% END Setup output directory structure
    ...
    displayLogItem('Miscellaneous parameters:', logFile);
    	...
    	displayLogItem(sprintf('\t%s', ['Part number:  ', num2str(partNum)]), logFile);
