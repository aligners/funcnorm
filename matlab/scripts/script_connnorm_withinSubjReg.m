function warps = script_connnorm_withinSubjReg(subjects, hems, varargin)
% FUNCTION warps = script_connnorm_withinSubjReg(subjects, hems, experiment, mmRes, fwhm, fs_surf, outDir, ...
...
%% Setup output directory structure
    ...
    alignDir = [outDir, 'alignment_withinSubjReg_', experiment, '_', num2str(lambda_areal), '/'];
    if isdir(alignDir) == 0
        mkdir(alignDir);
    else
...
for subjNum = 1:nsubj
	...
	t1size = ceil(size(T,1)/2);
	T1 = normalizeDataset(T(1:t1size, :));
	T2 = normalizeDataset(T((t1size+1):end, :));
	...
	alignLogFile = [logDir, 'log_alignment_1stHalf_to_2ndHalf_subjNum_', num2str(subjNum), '_subj_', subj, '.txt'];
	% Now run the algorithm
	displayLogItem(['Performing alignment (1st half to 2nd half) of subject #', num2str(subjNum), ' (', subj, ')'], logFile);
	...
	warp1stTo2nd = connnorm_register(sub1stHalf, sub2ndHalf, coords, zeros(3, numNodes, opsCoordsType), regularization, maxResolution, alignLogFile);
		displayLogItem(['Completed alignment (1st half to 2nd half) of subject #', num2str(subjNum), ' (', subj, ')'], logFile);

	% Now we need to output the warp file
	saveWarp(warp1stTo2nd, subj, '1stto2nd');
...
%% FUNCTION getWarpFilename
    function warpFilename = getWarpFilename(subj, hem, suffix)
        warpFilename = [tmpDir, 'warp_', subj, '_', hem, '_', suffix, '.bin'];
    end
%% END FUNCTION getWarpFilename


%% FUNCTION saveWarp
    function saveWarp(warp, subj, suffix)
        n = 1;
        for hemNum = 1:nhem
            hem = hems{hemNum};

            fp = fopen(getWarpFilename(subj, hem, suffix), 'wb');
